...
namespace CallingBotSample.Helpers
{
    ...
    public class GraphHelper : IGraph
    {
        ...
        /// <inheritdoc/>
        public async Task<object> CreateOnlineMeetingAsync()
        {
            try
            {
        ...
        public async Task<object> JoinScheduledMeeting(string meetingUrl)
        {
            try
            {
                    ...
                    var participants = new List<InvitationParticipantInfo>()
                {
                    new InvitationParticipantInfo
                    {
                        Identity = new IdentitySet
                        {
                            User = new Identity
                            {
                                DisplayName = this.users.ElementAt(0).DisplayName,
                                Id = this.users.ElementAt(0).Id
                            }
                        }
                    }
                };
